    ...
    public class BackwardSlicerTests
    {
        ...
        public void Bwslc_Issue_691()
        {
            ...
            while (bwslc.Step())
                ;
            //Assert.AreEqual(1, bwslc.Live.Count);
            Assert.AreEqual("(int32) (int16) (word16) ((word16) (((word16) d0 - 0x0020) * 0x00000002) * 0x00000002) + 0x0000A8B4", bwslc.JumpTableFormat.ToString());
            Assert.AreEqual("v3", bwslc.JumpTableIndex.ToString());
            Assert.AreEqual("(word16) d0", bwslc.JumpTableIndexToUse.ToString(), "Expression to use when indexing");
